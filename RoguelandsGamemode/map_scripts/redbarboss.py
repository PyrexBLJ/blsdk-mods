    ...
    if State.gift_box is not None and GameState.map_is_loaded:
        ...
        if (util.distance(hitloc, placeablehelper.static_mesh.get_location(State.gift_box[0].uobj)) < 150):
            ...
            if GameState.level_offset == 2:
                bsl.ItemPools = [unrealsdk.FindObject("ItemPoolDefinition", "GD_Itempools.EnemyDropPools.Pool_GunsAndGear_06_Legendary"), unrealsdk.FindObject("ItemPoolDefinition", "GD_Itempools.EnemyDropPools.Pool_GunsAndGear_06_Legendary"), unrealsdk.FindObject("ItemPoolDefinition", "GD_Itempools.EnemyDropPools.Pool_GunsAndGear_06_Legendary")]
            else:
